...
class ArnoldDataset(Dataset):
    ...
    def sample(self, batch_size, steps):
        ...
        while len(samples) < batch_size:
            ...
            act_idx = [1,2]
            demo_idx = np.random.randint(len(self.episode_dict[obj_idx][f'act{act_idx[0]}']), size=1)[0]
            obj_act_demo_tuple = (obj_idx, act_idx, demo_idx)
            if obj_act_demo_tuple not in sampled_idx:
                samples.append([self.episode_dict[obj_idx][f'act{act_idx[0]}'][demo_idx], self.episode_dict[obj_idx][f'act{act_idx[1]}'][demo_idx]])
