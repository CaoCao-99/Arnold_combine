...
@hydra.main(config_path='./configs', config_name='novel_state')
def main(cfg):
    ...
    if cfg.novel:
        if cfg.task != 'multi':
            train_dataset = ArnoldDataset(data_path=os.path.join(cfg.novel_state, cfg.task), task=cfg.task, obs_type=cfg.obs_type, novel=cfg.novel)
        else:
            train_dataset = ArnoldMultiTaskDataset(data_root=cfg.novel_state, obs_type=cfg.obs_type, novel=cfg.novel)
    else:
        if cfg.task == 'multi':
