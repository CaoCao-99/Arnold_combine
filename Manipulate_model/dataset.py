...
class ArnoldDataset(Dataset):
    def __init__(self, data_path, task, obs_type='rgb', offset_bound=np.array([-0.63, 0, -0.63, 0.63, 1.26, 0.63]), novel:bool=True):
        """
        Dataset structure: {
            object_id: {
                'act1': List,
        ...
        self.sample_weights = [0.0, 1.0] #[0.2, 0.8]
        self.obj_ids = []
        self.episode_dict = {}
...
class ArnoldMultiTaskDataset(Dataset):
    ...
    # task_list = [
    #     'pickup_object', 'open_drawer', 'close_drawer'
    # ]
    def __init__(self, data_root, obs_type='rgb', novel:bool=False, combine:bool=False):
        """
        Dataset structure: [
            {
                object_id: {
        ...
        super().__init__()
        self.novel=novel
        self.combine = combine  #추가
        self.data_root = data_root
        self.obs_type = obs_type
        self.task_dict = {}
        self.aug_data = ['pickup_object', 'open_drawer', 'close_drawer', 'open_cabinet', 'close_cabinet']
        if not novel:
            for task in self.task_list:
                print(f'Create dataset for {task}')
                self.task_dict[task] = ArnoldDataset(os.path.join(data_root, task, 'train'), task, obs_type)
        else:
            for task in self.aug_data:
                print(f'Create dataset for {task}')
                self.task_dict[task] = ArnoldDataset(os.path.join(data_root, task), task, obs_type)
                    
            # else:
            #     if self.combine:
            #         for task in self.task_list:
            #             if task not in aug_data:
            #                 print(f'Create dataset for {task}')
            #                 self.task_dict[task] = ArnoldDataset(os.path.join(data_root, task, 'train'), task, obs_type)
    ...
    def sample(self, batch_size, steps):
        ...
        while len(samples) < batch_size:
            if self.novel:
                task = random.choice(self.aug_data)
            else:
                task = random.choice(self.task_list)
            samples.append(self.task_dict[task].sample(1, steps)[0])
