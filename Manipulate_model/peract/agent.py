...
class PerceiverIO(nn.Module):
    def __init__(
        ...
        spatial_size = voxel_size // self.voxel_patch_stride # 120/5 = 24
        ...
        # 120x120x120x64 -> 120x120x120x1 decoder for translation Q-values
    ...
    def forward(
        ...
        # preprocess
        d0 = self.input_preprocess(ins)               # [B,10,120,120,120] -> [B,64,120,120,120]
        ...
        ins = self.patchify(d0)                       # [B,64,120,120,120] -> [B,64,20,20,20]
        ...
        # upsample layer
        u0 = self.up0(latents)                         # [B,64,120,120,120]

        # skip connection like in UNets
        u = self.final(torch.cat([d0, u0], dim=1))     # [B,64+64,120,120,120] -> [B,64,120,120,120]
        aaaa = u
        # translation decoder
        trans = self.trans_decoder(u)                  # [B,64,120,120,120] -> [B,1,120,120,120]
...
class PerceiverActorAgent():
    ...
    def load_model(self, path, gpu_id):
        checkpoint = torch.load(path, map_location=gpu_id)      #GPU 추가
    ...
    def predict(self, replay_sample: dict) -> dict:
        ...
        #continuous_trans = continuous_trans[0]                          #주석 처리
        bs = self._batch_size
        continuous_quat = []
        for i in range(bs):
            continuous_quat.append(discrete_euler_to_quaternion(rot_and_grip_indices[i][:3].detach().cpu().numpy(), #0 -> : 로 수정
    ...
    def update(self, step: int, replay_sample: dict, backprop: bool = True) -> dict:
        # cur_action_trans = replay_sample['cur_trans_action_indices'].int()       # 현재 주어진 GT scene에 해당하는 위치 값
        # cur_action_rot_grip = replay_sample['cur_trans_action_indices'].int()       # 현재 주어진 GT scene에 해당하는 위치 값
        ...
        if backprop:
            ...
            if self._with_state_head:
                ...
                # print(f'state_value : {state_value}, state_label : {state_label}')
        ...
        for i in range(bs):
            continuous_quat.append(discrete_euler_to_quaternion(rot_and_grip_indices[i][:3].detach().cpu().numpy(), #0 -> : 로 수정
                                                resolution=self._rotation_resolution))
            gt_continuous_quat.append(discrete_euler_to_quaternion(action_rot_grip[i][:3].detach().cpu().numpy(), # 0 -> : 로 변경 
                                                resolution=self._rotation_resolution))
        ...
        res = (bounds[:, 3:] - bounds[:, :3]) / self._voxel_size
        continuous_trans = bounds[:, :3] + res * coords_indices.int() + res / 2
        gt_continuous_trans = bounds[:, :3] + res * action_trans.int() + res / 2
        ...
        return {
            ...
            'voxel_grid': voxel_grid,
            'q_trans': self._softmax_q(q_trans),
            'trans_loss' : trans_loss[0].item(),
            'state_loss' : state_loss[0].item(),
            'rot_grip_loss' : rot_grip_loss[0].item(),
            'pred_action': {
                'trans': coords_indices,
                'continuous_quat' : continuous_quat,        #추가
                'continuous_trans': continuous_trans,
                'rot_and_grip': rot_and_grip_indices,
                'states': state_value
            },
            'expert_action': {
                ...
                'continuous_quat': gt_continuous_quat,   #추가
                'continuous_trans': gt_continuous_trans,   #추가
