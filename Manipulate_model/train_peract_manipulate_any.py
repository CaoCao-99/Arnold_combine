...
def prepare_batch(batch_data, cfg, lang_embed_cache, device):
    ...
    for data in batch_data:
        for k, v in data['obs_dict'].items():
            if k not in obs_dict:
                ...
                obs_dict[k].append(v)
        current_points.append(data['current_points'])
    ...
    current_action_coords = current_points[:, :3]
    current_action_indices = point_to_voxel_index(current_action_coords, cfg.voxel_size, cfg.offset_bound)
    ...
    cur_rot_action_quat = current_points[:, 3:]
    cur_rot_action_quat = normalize_quaternion(cur_rot_action_quat)
    cur_rot_action_indices = quaternion_to_discrete_euler(cur_rot_action_quat, cfg.rotation_resolution)
    cur_rot_grip_action_indices = np.concatenate([cur_rot_action_indices, gripper_open], axis=-1)
    ...
    # lang_goal_embs = [] #추가 및 수정
    # lang_goal_embs.append(lang_embed_cache.get_lang_embed(language_instructions)) 
    lang_goal_embs = lang_embed_cache.get_lang_embed(language_instructions)

    inp = {}
    inp.update(obs_dict)
    inp.update({
        # 'cur_trans_action_indices' : current_action_indices,
        # 'cur_rot_grip_action_indices' : cur_rot_action_indices,
...
@hydra.main(config_path='./configs', config_name='any_state')
def main(cfg):
    ...
    if cfg.novel:
        if cfg.task != 'multi':
            train_dataset = ArnoldDataset(data_path=os.path.join(cfg.any_state, cfg.task), task=cfg.task, obs_type=cfg.obs_type, novel=cfg.novel)
        else:
            train_dataset = ArnoldMultiTaskDataset(data_root=cfg.any_state, obs_type=cfg.obs_type, novel=cfg.novel)
    else:
        if cfg.task == 'multi':
    ...
    for iteration in range(start_step, cfg.steps):
        ...
        print(f'instruction : {language_instructions}')
