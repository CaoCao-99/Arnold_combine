...
#import pdb
#pdb.set_trace()
simulation_app, simulation_context, _ = get_simulation(headless=True, gpu_id=2, high_quality=False)
...
def main(cfg):
    ...
    if not cfg.use_gt[1]:
        ...
        agent2 = load_ckpt(cfg, agent2, cfg.checkpoint_dir_manipulate, device)
        print(f'cfg.checkpoint_dir_grasp : {cfg.checkpoint_dir_manipulate}\n\n')
#    else:
...
#    pattern = re.compile(r'.*(160000|170000|180000|190000|200000)\.pth$')
 #   ckpts_list = [f for f in os.listdir(cfg.checkpoint_dir) if pattern.match(f)]
    ...
    for ckpt_name in ckpts_list:
        ...
        agent1 = load_ckpt(cfg, agent1, os.path.join(cfg.checkpoint_dir, ckpt_name), device)
