...
import logging
import numpy as np
import torch.nn as nn  # nn 모듈을 임포트합니다.
import os
import torch
...
# 정보를 저장할 폴더 경로
#output_folder = 'output_folder_sm'
...
def load_agent(cfg, device, ckpt_dir):
    ...
    if cfg.model == 'cliport6d':
        ...
        logger.info(f'ck_path : {checkpoint_path}')
        print(f'ck_path : {checkpoint_path}')
        agent.module.load_model(checkpoint_path)

        lang_encoder = create_lang_encoder(cfg, device=device)
        lang_embed_cache = InstructionEmbedding(lang_encoder)
        # DataParallel 객체로 다시 감싸기
        # agent = nn.DataParallel(agent, device_ids=[0, 1, 2, 3])  # 병렬로 사용할 GPU 번호 목록 지정
    elif 'bc_lang' in cfg.model:
        from train_bc_lang import create_agent, create_lang_encode
        agent = create_agent(cfg, device=device)
...
#        agent.to(device)
    ...
    agent1, lang_embed_cache1 = load_agent(cfg, device=device, ckpt_dir=cfg.checkpoint_dir_grasp)
    agent2, lang_embed_cache2 = load_agent(cfg, device=device, ckpt_dir=cfg.checkpoint_dir_manipulate)
    #agent.to(device)
    agent1.to(device)
    agent2.to(device)
    import pdb
#    pdb.set_trace()
    ...
    for task in task_list:
        file_path = os.path.join(cfg.output_folder, f'{task}.txt')  # task 이름으로 파일명 설정
                # output_folder가 없는 경우 생성합니다.
        if not os.path.exists(cfg.output_folder):
            os.makedirs(cfg.output_folder)
        with open(file_path, 'w') as file:
            file.write(f'Task 명: {task}\n')
            file.write('_' * 70 + '\n')  # 구분선 추가
 #       import pdb
#        pdb.set_trace()
        ...
        for eval_split in cfg.eval_splits:
            ...
            with open(file_path, 'a') as file:
                file.write(f'Evaluating {task} {eval_split}\n')
                file.write('_' * 70 + '\n')  # 구분선 추가
    ...
    #                file.write(f"scene : {fname}\n") # scene 이름
            ...
            with open(file_path, 'a') as file:
                file.write(f'{task} {eval_split} score: {correct/total*100:.2f}\n\n')    #Score
